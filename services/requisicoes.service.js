	...
	actions: {
		...
		list: {
			...
			async handler(ctx) {
				...
				// helper p/ mesclar e remover duplicatas por req_id
				const mergeById = (a = [], b = []) => {
					const map = new Map();
					[...a, ...b].forEach((r) => {
						const id = r?.req_id ?? r?.id ?? r?.reqId;
						if (!id) return;
						// prioriza o objeto com mais campos (ex.: já com itens/decisões, se vierem em algum fluxo)
				...
				if (hasGlobal || canEdit) {
					...
					// filtra por categorias permitidas (sua lógica original)
					...
					if (reqIds.length === 0) {
						this.logger.debug("[requisicoes.list] nenhum req visível p/ allowed categories:", [...allowed]);
						// NÃO retorna ainda: vamos somar as minhas
						headers = [];
					} else {
						headers = await this.adapter.find({ query: { req_id: reqIds }, sort: ["-req_id"] });
					}
				} else {
					// sem manage_category -> só veria as próprias (fluxo antigo)
					headers = await this.adapter.find({ query: { req_fk_user: userId }, sort: ["-req_id"] });
				}

				// >>> NOVO: sempre inclua as requisições do próprio usuário
				const myHeaders = await this.adapter.find({ query: { req_fk_user: userId }, sort: ["-req_id"] });

				// normaliza eventuais instâncias
				const toPlain = (arr = []) => (arr || []).map(h => {
					try { if (h && typeof h.toJSON === "function") return h.toJSON(); } catch (e) { }
					return h;
				});

				const headersPlain = toPlain(headers);
				const myHeadersPlain = toPlain(myHeaders);

				// União: o resultado final SEMPRE contém as minhas
				const headersMerged = mergeById(headersPlain, myHeadersPlain);

				this.logger.debug("[requisicoes.list] headersMerged sample:", headersMerged.length ? headersMerged[0] : null);

				// A partir daqui, tudo usa headersMerged
				const reqIds = headersMerged.map(h => (h.req_id ?? h.id ?? h.reqId)).filter(Boolean);
				this.logger.debug("[requisicoes.list] reqIds (merged):", reqIds.slice(0, 50));
				...
				if (ctx.params.includeItems) {
					...
					this.logger.debug("[requisicoes.list] itens loaded:", Object.keys(itensByReq).length);
				}
				...
				if (ctx.params.includeDecisions) {
					...
					this.logger.debug("[requisicoes.list] decisoes loaded:", Object.keys(decisByReq).length);
				}
				...
				// preencher nomes em lote (igual à sua lógica)
				const preencherNomes = async (arr) => {
					...
					return arr.map(h => {
						...
						return {
							...h,
							req_user_nome: h.req_user_nome ?? nameFromDb ?? h.user_nome ?? nameFromMeta ?? null
						};
					});
				};

				const filled = await preencherNomes(normalized);
				this.logger.debug("[requisicoes.list] returning sample (merged):", filled.length ? filled[0] : null);
				return filled;
			}
		},
		...
		create: {
			...
			async handler(ctx) {
				...
				return await sequelize.transaction(async tx => {
					...
					// tenta obter nome do usuário do ctx.meta (se autenticação injetou) ou do model User
					let userName = null;
					if (ctx?.meta?.user) {
		...
		decidir: {
			...
			async handler(ctx) {
				...
				// valida permissões de categoria (backend authoritative check)
				const { allowed, hasGlobal } = this._getAllowedCategoryIds(ctx);
				if (!hasGlobal) {
					// busca itens da requisicao e verifica categorias
					const itensDb = await RequisicaoItem.findAll({ where: { rqi_fk_requisicao: id }, raw: true });
					if (!itensDb || itensDb.length === 0) {
						throw new Error("Requisição sem itens ou não encontrada.");
					}

					// coleta categorias dos materiais envolvidos
					...
					for (const it of itensDb) {
						...
						if (!catId) {
							// se material sem categoria definida, bloqueia por segurança
							throw new Error(`Categoria não definida para material ${material.mat_nome}.`);
						}
						categoriasDaReq.add(catId);
					}

					// verifica se todas as categorias estão dentro do allowed set
				...
				// se passou nas validações acima, executa a transação que cria a decisão e atualiza cabeçalho
				return await sequelize.transaction(async tx => {
					const header = await this.adapter.model.findByPk(id, { transaction: tx });
					if (!header) throw new Error("Requisição não encontrada.");
