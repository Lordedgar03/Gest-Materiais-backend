...
module.exports = {
  ...
  actions: {
    /**
     * GET /vendas
     * Query: status?, data_ini?, data_fim?, caixa?, q?
     */
    list: {
      rest: "GET /vendas",
      cache: false,
      params: {
        ...
        data_ini: { type: "string", optional: true },
    ...
    /**
     * POST /vendas/requisitar
     * Body: { ven_cliente_nome, itens:[{ material_id, quantidade }] }
     * Usa o fluxo oficial de REQUISIÇÕES: create -> decidir(Aprovar) -> atender
     * Gera Recibo (tipo "Requisição de Material") para impressão no caixa.
     */
    requisitar: {
      rest: "POST /vendas/requisitar",
      params: {
        ven_cliente_nome: { type: "string", min: 1 },
        itens: {
      ...
      async handler(ctx) {
        ...
        const cx = await ctx.call("caixas.aberto").catch(() => null);
        if (!cx) throw new Error("Abra o caixa do dia antes de registrar requisições.");

        const { ven_cliente_nome, itens } = ctx.params;

        return sequelize.transaction(async tx => {
          // 1) cria requisição
          const created = await ctx.call("requisicoes.create", {
            req_fk_user: userId,
            req_observacoes: `Origem: Vendas (cliente: ${ven_cliente_nome})`,
            itens: itens.map(i => ({
              rqi_fk_material: Number(i.material_id),
              rqi_quantidade: Number(i.quantidade),
              rqi_descricao: null
            }))
          }, { meta: ctx.meta, transaction: tx });

          const reqId = created?.data?.req_id ?? created?.data?.id ?? created?.req_id;
          if (!reqId) throw new Error("Falha ao criar a requisição.");

          // 2) aprovar
          await ctx.call("requisicoes.decidir", { id: reqId, tipo: "Aprovar", motivo: "PDV" }, { meta: ctx.meta, transaction: tx });

          // 3) atender todos
          const reqItens = await sequelize.models.RequisicaoItem.findAll({
            where: { rqi_fk_requisicao: reqId }, raw: true, transaction: tx
          });
          if (!reqItens.length) throw new Error("Requisição criada sem itens.");

          await ctx.call("requisicoes.atender", {
            id: reqId,
            itens: reqItens.map(ri => ({ rqi_id: ri.rqi_id, quantidade: ri.rqi_quantidade }))
          }, { meta: ctx.meta, transaction: tx });

          // 4) recibo (0€ – apenas comprovativo de requisição de material)
          const reqHeader = await sequelize.models.Requisicao.findByPk(reqId, { raw: true, transaction: tx });
          const rec = await Recibo.create({
            rec_fk_user: userId,
            rec_tipo: "Requisição de Material",
            rec_total: 0,
            rec_ref: reqHeader?.req_codigo || `REQ-${String(reqId).padStart(6,"0")}`,
            rec_fk_requisicao: reqId,
            rec_cliente_nome: ven_cliente_nome,
            data: new Date()
          }, { transaction: tx });

          return {
            success: true,
            message: "Requisição criada, aprovada e atendida.",
            requisicao: {
              req_id: reqId,
              req_codigo: reqHeader?.req_codigo
            },
            recibo: {
              rec_id: rec.rec_id,
              pdf_hint: `/api/recibos/${rec.rec_id}/pdf`
            }
          };
        });
      }
