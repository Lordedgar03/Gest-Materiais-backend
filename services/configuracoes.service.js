...
const ConfiguracaoModel = require("../models/configuracao.model"); // <-- importa o modelo específico
...
module.exports = {
  ...
  actions: {
    ...
    list: {
      ...
      cache: { ttl: 60 },
      async handler() {
        return this.adapter.model.findAll({ order: [["cfg_chave", "ASC"]], raw: true });
      },
    },
    ...
    get: {
      ...
      async handler(ctx) {
        const row = await this.adapter.model.findOne({ where: { cfg_chave: ctx.params.chave }, raw: true });
        return row || null;
      },
    },

    // GET /configuracoes/value/:chave
    getValue: {
      ...
      async handler(ctx) {
        ...
        const row = await this.adapter.model.findOne({ where: { cfg_chave: chave }, raw: true });
    ...
    // POST /configuracoes/bulk
    bulkUpsert: {
      rest: "POST /bulk",
      params: { items: { type: "array", items: "object", min: 1 } },
      async handler(ctx) {
